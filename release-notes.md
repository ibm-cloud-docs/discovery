...
- Trained collections return a `score` in the results of a natural language query. Starting 30 November 2017, the way that `score` is calculated changed. The `score` indicates the relative relevancy between query results, and therefore should not be used as a threshold. Instead, use the `confidence`, which indicates the relevance of the result as compared to the trained model, to set thresholds. See [Confidence scores](/docs/services/discovery/train-tooling.html#confidence) for more information on setting thresholds.
