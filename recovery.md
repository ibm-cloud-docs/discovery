...
If you also import documents by doing scheduled crawls of external data sources, you should retain your data source credentials externally so that you can reestablish your data sources quickly. See [Connecting to data sources](/docs/discovery?topic=discovery-sources) for the list of available sources and the credentials needed for each one.
...
If you are using Smart Document Understanding (SDU), you will have models associated with your configuration. To avoid loss of this information you should [export your models](/docs/discovery?topic=discovery-sdu#import), back them up and store them locally. SDU models have the file extension of `.sdumodel`.
...
-  If you had any custom configurations set up in {{site.data.keyword.discoveryshort}}, you'll need to [re-create those custom configurations](/docs/discovery?topic=discovery-configservice#when-you-need-a-custom-configuration). 
...
-  If you had custom query expansion, you will need to [add your query expansions](https://{DomainName}/apidocs/discovery#create-or-update-expansion-list){: external} for each collection as well. 
-  If you were using any custom entity models from Watson Knowledge Studio (WKS) for enrichment, you'll need to [re-import that model](/docs/discovery?topic=discovery-configservice#custom-entity-model) into your {{site.data.keyword.discoveryshort}} instance. 
...
After collections have been restored, you can begin the process of [recreating your relevancy training models](/docs/discovery?topic=discovery-improving-result-relevance-with-the-api). 
...
In case of an unanticipated loss of data, you may lose your scheduled crawls of external data sources. See [Connecting to data sources](/docs/discovery?topic=discovery-sources) for the list of available sources.

To restore your external data, you'll need to reestablish your connections to these data sources, and then re-crawl them.

To find the data source credentials you stored previously, follow the instructions for your chosen data source in [Connecting to Data Sources](/docs/discovery?topic=discovery-sources). This will allow you to re-connect to your data sources, and get the data imported into {{site.data.keyword.discoveryshort}}.
